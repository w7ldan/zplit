...
async function loadSettlement(database: Database, groupId: string, settlementId: string): Promise<GroupSettlementPresentation> {
  ...
  const [participants, proof] = await Promise.all([
  ...
  return settlementPresentation(settlement, participants, proof[0] ?? null);
}
