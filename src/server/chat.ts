...
import { chatMessages, chatThreads, groupMemberships, groupParticipants, organizationMemberships, users } from "@/db/schema";
...
async function listMessages(database: Database, scope: ChatScope, threadId: string, viewerUserId: string, canSend: boolean, canModerate: boolean, before?: unknown) {
  ...
  return { messages, nextCursor: hasMore && oldest ? encodeChatCursor(oldest) : null };
}
...
export async function getOrganizationChat(database: Database, organizationId: string, viewerUserId: string, before?: unknown): Promise<ChatViewDto> {
  ...
  const thread = await findThread(database, scope);
  const page = thread ? await listMessages(database, scope, thread.id, viewerUserId, access.can("chat.send"), access.can("chat.moderate"), before) : { messages: [], nextCursor: null };
...
export async function getGroupChat(database: Database, groupId: string, viewerUserId: string, before?: unknown): Promise<ChatViewDto> {
  ...
  const thread = await findThread(database, scope);
  const page = thread ? await listMessages(database, scope, thread.id, viewerUserId, true, access.canManageGroup, before) : { messages: [], nextCursor: null };
