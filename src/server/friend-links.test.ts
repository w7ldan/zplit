...
vi.mock("server-only", () => ({}));
vi.mock("@/server/notifications", () => ({ createNotificationInDatabase: mocks.createNotificationInDatabase }));
vi.mock("@/server/realtime", () => ({ publishRealtimeEvent: mocks.publishRealtimeEvent }));
vi.mock("@/auth/require-session", () => ({ requireSession: mocks.requireSession }));
vi.mock("@/db/client", () => ({ getDatabase: mocks.getDatabase }));

import { cancelFriendLinkRequest, createFriendLinkRequest, respondToFriendLinkRequest } from "./friend-links";
...
function builder(result: unknown) {
  ...
  for (const method of ["from", "innerJoin", "leftJoin", "where", "limit", "for", "set", "values"]) current[method] = vi.fn(() => chain);
  current.returning = vi.fn(() => Promise.resolve(result));
  chain.then = (resolve, reject) => Promise.resolve(result).then(resolve, reject);
  return chain;
}

type FakeDatabase = Database & { transactionDb: { update: ReturnType<typeof vi.fn> } };
...
describe("Friend ↔ Zplit-user linking", () => {
  ...
  it("cancels only a pending owner request and refreshes the target Inbox", async () => {
    ...
    expect(mocks.publishRealtimeEvent).toHaveBeenCalledWith(targetId, expect.objectContaining({ type: "notification.state.changed" }));
  });
});
