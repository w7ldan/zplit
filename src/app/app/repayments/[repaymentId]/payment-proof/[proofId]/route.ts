...
import {
  ...
  PAYMENT_PROOF_UNAVAILABLE_MESSAGE,
...
export async function GET(request: Request, { params }: { params: Promise<{ repaymentId: string; proofId: string }> }) {
  const session = await getAuth().api.getSession({ headers: await headers() });
  if (!session) return new Response("Unauthorized", { status: 401, headers: PAYMENT_PROOF_READ_HEADERS });

  const { repaymentId, proofId } = await params;
  try {
    const proof = await getRepaymentPaymentProof(getDatabase(), session.user.id, repaymentId, proofId);
    ...
    return new Response(PAYMENT_PROOF_UNAVAILABLE_MESSAGE, { status: 404, headers: PAYMENT_PROOF_READ_HEADERS });
  }
}

export async function DELETE(request: Request, { params }: { params: Promise<{ repaymentId: string; proofId: string }> }) {
  const session = await getAuth().api.getSession({ headers: await headers() });
  if (!session) return new Response("Unauthorized", { status: 401, headers: PAYMENT_PROOF_READ_HEADERS });
  ...
  const { repaymentId, proofId } = await params;
  try {
    const deleted = await deleteRepaymentPaymentProof(getDatabase(), session.user.id, repaymentId, proofId);
    if (!deleted) return new Response(PAYMENT_PROOF_UNAVAILABLE_MESSAGE, { status: 404, headers: PAYMENT_PROOF_READ_HEADERS });
