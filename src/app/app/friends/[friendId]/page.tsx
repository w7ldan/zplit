...
type FriendRecordSession = Awaited<ReturnType<typeof requireSession>>;

async function loadFriendRecordData(session: FriendRecordSession, friendId: string, query: FriendSearchParams | undefined) {
  try {
    const database = getDatabase();
    const { ledger: repository } = await getAuthenticatedLedger(session);
    const friend = await repository.getFriend(friendId);
    const [friendBalances, shareStatus, eligibleReceipts, selectedReceiptIds, sharedDestinations, expenseSharePage, repaymentPage, friendLinkStatus] = await Promise.all([
    ...
    return {
      ...
      eligibleReceipts,
      selectedReceiptIds,
      sharedDestinationNames: sharedDestinations.map((destination) => destination.name),
      expenseSharePage,
      repaymentPage,
      friendLinkStatus,
...
type FriendRecordData = Awaited<ReturnType<typeof loadFriendRecordData>>;

function FriendRecordHistory({ data, query }: { data: FriendRecordData; query: FriendSearchParams | undefined }) {
  const { friend, expenseSharePage, repaymentPage } = data;
...
function FriendRecordContent({ data, query }: { data: FriendRecordData; query: FriendSearchParams | undefined }) {
    ...
    <section className="app-page friend-record" id="top">
      ...
      <div className="editorial-grid editorial-shell friend-record__layout">
        <div className="friend-record__intro">
          <div className="friend-record__title"><p className="technical-label">Friend · editable record</p><h1>{friend.name}</h1></div>
        ...
        <div className="friend-record__workspace">
          <div className="friend-record__form">
            ...
            <FriendForm action={updateFriendAction.bind(null, friend.id)} mode="edit" initialValues={{ name: friend.name, phoneNumber: friend.phoneNumber ?? "", notes: friend.notes ?? "" }} />
            <FriendLinkSection status={friendLinkStatus} search={searchFriendLinkUserOptions} action={createFriendLinkRequestAction.bind(null, friend.id)} cancelAction={friendLinkStatus.status === "pending" ? cancelFriendLinkRequestAction.bind(null, friend.id, friendLinkStatus.requestId) : undefined} unlinkAction={friendLinkStatus.status === "linked" ? unlinkFriendLinkAction.bind(null, friend.id) : undefined} />
          ...
          <FriendShareLink status={{ status: shareStatus.status, expiresAt: shareStatus.expiresAt?.toISOString() ?? null }} phoneNumber={friend.phoneNumber} createAction={createDebtorShareLinkAction.bind(null, friend.id)} revokeAction={revokeDebtorShareLinkAction.bind(null, friend.id)} updateSelectionAction={updateDebtorShareReceiptSelectionAction.bind(null, friend.id)} eligibleReceipts={eligibleReceipts} selectedReceiptIds={selectedReceiptIds} sharedDestinationNames={sharedDestinationNames} />
        </div>
...
export default async function FriendRecordPage({ params, searchParams }: { params: Promise<{ friendId: string }>; searchParams?: Promise<FriendSearchParams> }) {
  ...
  const data = await loadFriendRecordData(session, friendId, query);

  return <FriendRecordContent data={data} query={query} />;
}
