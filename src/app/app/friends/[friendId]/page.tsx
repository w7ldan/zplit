...
import {
  archiveFriendAction,
  cancelFriendLinkRequestAction,
  createFriendLinkRequestAction,
  restoreFriendAction,
  searchFriendLinkUserOptions,
  unlinkFriendLinkAction,
  undoFriendArchiveAction,
  updateFriendAction,
} from "../actions";
import {
  createDebtorShareLinkAction,
  revokeDebtorShareLinkAction,
  updateDebtorShareReceiptSelectionAction,
} from "./share-actions";
...
async function loadFriendRecordData(
  session: FriendRecordSession,
  friendId: string,
  query: FriendSearchParams | undefined,
) {
  try {
    ...
    const [
      ...
      repository.listFriendExpenseShareRecords(friend.id, {
        page: first(query?.expensePage),
      }),
      repository.listRepaymentRecords({
        friendId: friend.id,
...
function FriendRecordHistory({
  data,
  query,
}: {
  data: FriendRecordData;
  query: FriendSearchParams | undefined;
}) {
    ...
    <>
      ...
      >
        ...
        {expenseSharePage.items.length > 0 ? (
            ...
            {expenseSharePage.items.map((share) => (
              ...
              >
                <div className="record-history__primary">
                  ...
                  <h3>
                    <Link href={"/app/expenses/" + share.expenseId}>
                ...
                <span
                  className={
                    "record-history__state" +
                    (share.settled ? " record-history__state--settled" : "")
                  }
                >
                  {share.settled ? "SETTLED" : "OPEN"}
                </span>
          ...
          <div className="ledger-empty">
            <h3>No expense shares recorded for this friend yet.</h3>
          </div>
        )}
        <RecordPagination
          page={expenseSharePage.page}
          pageSize={expenseSharePage.pageSize}
          totalItems={expenseSharePage.totalItems}
          totalPages={expenseSharePage.totalPages}
          href={historyHref}
      ...
      >
        ...
        {repaymentPage.items.length > 0 ? (
              ...
              >
                ...
                <div className="record-history__values">
                  ...
                  <span>
                    ...
                    <strong>{repayment.paymentMethod ?? "—"}</strong>
                  </span>
                  <span>
          ...
          <div className="ledger-empty">
            <h3>No repayments recorded for this friend yet.</h3>
          </div>
        )}
        <RecordPagination
          page={repaymentPage.page}
          pageSize={repaymentPage.pageSize}
          totalItems={repaymentPage.totalItems}
          ...
          <section
            className={
              ...
              (balance.assignedAmount === 0
                ...
                : balance.outstandingAmount === 0
                  ? " friend-record__balance--settled"
                  : "")
            }
        ...
        <div className="friend-record__workspace">
          <div className="friend-record__form">
            ...
            <FriendLinkSection
              ...
              cancelAction={
                friendLinkStatus.status === "pending"
                  ? cancelFriendLinkRequestAction.bind(
                      null,
                      friend.id,
                      friendLinkStatus.requestId,
                    )
                  : undefined
              }
              unlinkAction={
                friendLinkStatus.status === "linked"
                  ? unlinkFriendLinkAction.bind(null, friend.id)
                  : undefined
              }
            />
            <FriendArchiveForm
              action={(archived ? restoreFriendAction : archiveFriendAction).bind(
                null,
                friend.id,
              )}
              archived={archived}
