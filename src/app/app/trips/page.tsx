        ...
        <div className="records-workspace">
          <div className="records-workspace__toolbar">
            ...
            <LiveRecordFilters
              ...
              clearHref={filtered ? recordHref("/app/trips", params, { q: undefined, page: undefined }) : undefined}
              resultStatus={`${tripPage.totalItems} trip${tripPage.totalItems === 1 ? "" : "s"} found.`}
          ...
          <div className="ledger-list" id="record-list">
            ...
            {tripPage.items.length > 0 ? (
              ...
              <div className="ledger-empty">
                <h2>{filtered ? "No matching Trips." : "No Trips yet."}</h2>
                <p>
                  {filtered
                    ? "Try a different Trip name."
                    : "Group your first related outings before adding more ledger records."}
                </p>
                {filtered ? null : (
                  <Link
                    className="text-link"
                    ...
                    data-task-trigger="trip-create"
                  >
                    Add trip <span aria-hidden="true">→</span>
                  </Link>
                )}
              </div>
            )}
            <RecordPagination
              ...
              totalItems={tripPage.totalItems}
              totalPages={tripPage.totalPages}
              href={recordHref("/app/trips", params)}
            />
          </div>
        </div>
      </div>
