...
export default async function TripRecordPage({
  params,
  searchParams,
}: {
  ...
  const copySummary = [
    `${trip.name} · ${tripDateSummary(trip.startsOn, trip.endsOn)}`,
    "",
    ...friendSettlements.map((friend) =>
      friend.outstandingAmount > 0
        ? `${friend.friendName}: ${formatRupiah(friend.outstandingAmount)} outstanding`
        : `${friend.friendName}: settled`,
    ),
  ].join("\n");
  return (
      ...
      <div className="editorial-grid editorial-shell trip-record__layout">
        <div className="trip-record__intro">
          ...
          <div className="trip-record__actions">
            ...
            <Link className="trip-record__back" href="/app/trips">
              ← Back to Trips
            </Link>
          </div>
        </div>
        {query?.saved === "1" ? (
          <RecordConfirmation queryKey="saved" message="Trip changes saved." />
        ) : null}
        <section className="trip-record__summary" aria-label="Trip summary">
          <div className="trip-record__meta" aria-label="Trip metadata">
            <div>
              ...
              <CalendarDateRange startsOn={trip.startsOn} endsOn={trip.endsOn} />
            </div>
            <div>
          ...
          <section className="trip-record__financials" aria-label="Trip financial summary">
            ...
            <div>
              <span className="technical-label">Assigned to friends</span>
              <strong>{formatRupiah(summary.totalAssignedAmount)}</strong>
            </div>
            <div>
              <span className="technical-label">Your portion</span>
              <strong>{formatRupiah(summary.ownerPortionAmount)}</strong>
            </div>
            <div>
          ...
          <section className="trip-record__settlement" aria-labelledby="trip-settlement-heading">
            <div className="trip-record__section-heading">
              <div>
                <p className="technical-label">SETTLE BALANCES</p>
                <h2 id="trip-settlement-heading">Friend balances</h2>
              </div>
            </div>
            {friendSettlements.length > 0 ? (
              <div className="trip-record__settlement-list">
                {friendSettlements.map((friend) => (
                  <div className="trip-record__settlement-row" key={friend.friendId}>
                    <div>
                      <strong>{friend.friendName}</strong>
                      {friend.outstandingAmount > 0 ? (
                        <span>{formatRupiah(friend.outstandingAmount)} outstanding</span>
                      ) : (
                        <span className="technical-label">Settled</span>
                      )}
                    </div>
                    {friend.outstandingAmount > 0 ? (
                      <Link
                        className="action-link action-link--quiet"
                        href={`/app/repayments?create=1&friendId=${friend.friendId}&tripId=${trip.id}`}
                      >
              ...
              <div className="ledger-empty">
                <h3>No friend shares in this Trip yet.</h3>
                <p>Settlement appears after an expense is shared with a friend.</p>
              </div>
            )}
          </section>
        ...
        <section className="trip-record__outings" aria-labelledby="trip-outings-heading">
          <div className="trip-record__section-heading">
            <div>
              ...
              <h2 id="trip-outings-heading">Outings</h2>
            </div>
            <span className="technical-label">{outingPage.totalItems} entries</span>
          </div>
          {outingPage.items.length > 0 ? (
            outingPage.items.map((outing) => (
              <OutingRow
                key={outing.id}
                outing={outing}
            ...
            <div className="ledger-empty">
              <h3>No outings in this Trip yet.</h3>
              <p>Add an outing to start grouping records.</p>
              <Link
                className="text-link"
                href={`/app/outings?create=1&trip=${trip.id}`}
              >
          ...
          <RecordPagination
            page={outingPage.page}
            pageSize={outingPage.pageSize}
            totalItems={outingPage.totalItems}
            totalPages={outingPage.totalPages}
