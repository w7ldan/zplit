...
import {
  ...
  removeRepaymentAllocationAction,
  replaceRepaymentAllocationsAction,
  searchFriendOptions,
  undoRepaymentAllocationAction,
...
export default async function OrganizationRepaymentPage({
  params,
  searchParams = Promise.resolve({}),
}: {
  params: Promise<{ organizationId: string; repaymentId: string }>;
  ...
  let plan;
  try {
    plan = await access.ledger.getRepaymentAllocationPlan(repaymentId, {
      q: first(query.q),
  ...
  const [impact, friendRows, friendContext, methods, proof] = await Promise.all(
    [
      access.ledger.getRepaymentDeletionImpact(repaymentId),
      ...
      <div className="editorial-grid editorial-shell repayment-record__layout">
        <div className="repayment-record__intro">
          <p className="technical-label">
            Organization Repayment · allocate received money
          </p>
          <h1>{plan.friendName}</h1>
          <Link className="repayment-record__back" href={`${base}/repayments`}>
            ← Back to repayments
          </Link>
        </div>
        <div className="repayment-record__tasks">
          <div className="repayment-record__primary-task">
            {canEdit ? (
              <RepaymentAllocationEditor
                action={replaceRepaymentAllocationsAction.bind(
                  null,
                  organizationId,
                  plan.id,
                )}
                ...
                removeAction={removeRepaymentAllocationAction.bind(
                  null,
                  organizationId,
                )}
                undoAction={undoRepaymentAllocationAction.bind(
                  null,
                  organizationId,
                )}
                basePath={base}
              ...
              <p className="app-page__lede">
                Repayment allocation is read-only for your current Organization
                access.
              </p>
            )}
            <RepaymentPaymentProof
              repaymentId={plan.id}
              initialPaymentProof={proof}
          ...
          <aside className="repayment-record__sidebar">
            <div className="repayment-record__controls">
              <div className="repayment-record__meta">
                <div>
                  <span className="technical-label">Received</span>
                  <strong>{formatRupiah(plan.amount)}</strong>
                </div>
                <div>
                  <span className="technical-label">Applied</span>
                  <strong>{formatRupiah(plan.allocatedAmount)}</strong>
                </div>
                <div>
                  <span className="technical-label">Needs allocation</span>
                  <strong>{formatRupiah(plan.unallocatedAmount)}</strong>
                </div>
                <div>
              ...
              {canEdit ? (
                <RepaymentForm
                  action={updateRepaymentAction.bind(
                    null,
                    organizationId,
                    plan.id,
                  )}
                  ...
                  recentPaymentMethods={methods}
                  mode="edit"
                  friendLocked={plan.allocatedAmount > 0}
                  initialFriendContext={formContext}
                  loadFriendContext={loadRepaymentFriendContext.bind(
                    null,
                    organizationId,
                  )}
              ...
              {access.can("repayments.delete") ? (
                <DeleteRecordForm
                  action={deleteRepaymentAction.bind(
                    null,
                    organizationId,
                    plan.id,
                  )}
