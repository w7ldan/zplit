...
async function loadOrganizationExpenses(
  ...
  const outingOptions = outingRows.map((outing) => ({
    id: outing.id,
    label: outing.title,
    group: outing.recent ? "Recent" : undefined,
  }));
  const page = await access.ledger.listExpenseRecords({
    q: first(query.q),
    outingId,
    month: first(query.month),
    assignment: first(query.assignment),
...
function OrganizationExpenseList({
  data,
  base,
  path,
  query,
}: {
  data: OrganizationExpensesData;
  base: string;
  path: string;
  query: Record<string, string | string[] | undefined>;
}) {
    ...
    <div className="ledger-list" id="record-list">
      ...
      {page.items.length ? (
        ...
        <div className="ledger-empty">
          ...
          <p>
            {filtered ? "Try a different search or clear the filters." : "Every expense belongs to an outing."}
          </p>
        </div>
      )}
      <RecordPagination
...
function OrganizationExpenseCreatePanel({
  data,
  organizationId,
  base,
}: {
  ...
  return (
    ...
    >
      {outingOptions.length ? (
        <ExpenseForm
          action={createExpenseAction.bind(null, organizationId)}
          outings={outingOptions}
          searchOutings={searchOutingOptions.bind(null, organizationId)}
          initialValues={{
            description: "",
            amountRupiah: "",
            outingId: outingId ?? "",
...
function OrganizationExpensesContent({
  data,
  organizationId,
  base,
  path,
    ...
    <section className="app-page expenses-page" id="top">
      <div className="editorial-shell app-page__layout">
        <div className="app-page__header">
          <div>
            <p className="technical-label">Organization expenses · money paid</p>
            <h1>Expenses</h1>
            <p className="app-page__lede">
              Record shared spending and assign what each friend owes.
            </p>
          </div>
          {canCreate ? (
            <Link
        ...
        <LiveRecordFilters
          ...
          search={{
            ...
            placeholder: "Description or outing",
            value: filters.q ?? "",
          }}
          selects={[
            {
              ...
              search: searchOutingFilterOptions.bind(null, organizationId),
            },
            {
              name: "assignment",
              label: "Assignment",
          ...
          resultStatus={
            page.totalItems + " expense" + (page.totalItems === 1 ? "" : "s") + " found."
          }
      ...
      <OrganizationExpenseCreatePanel
        data={data}
