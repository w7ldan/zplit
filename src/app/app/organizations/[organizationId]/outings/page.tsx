...
async function loadOrganizationOutings(
  query: Record<string, string | string[] | undefined>,
  access: OrganizationOutingsAccess,
) {
  ...
  const filters = normalizeOutingFilters({
    q: first(query.q),
  ...
  const selectedTrip =
    tripFilter && tripFilter !== "unassigned"
      ? await access.ledger.getTrip(tripFilter).catch(() => undefined)
      : undefined;
  const page = await access.ledger.listOutingRecords({
    q: first(query.q),
    month: first(query.month),
    page: first(query.page),
    timezoneOffsetMinutes,
    ...(tripFilter ? { trip: tripFilter } : {}),
  });
  return {
    tripFilter,
    filters,
    ...
    outingOptions: [
      ...
      ...(selectedTrip ? [{ value: selectedTrip.id, label: selectedTrip.name }] : []),
    ],
...
function OrganizationOutingList({
  data,
  base,
  path,
  query,
}: {
  data: OrganizationOutingsData;
  base: string;
  path: string;
  query: Record<string, string | string[] | undefined>;
}) {
    ...
    <div className="ledger-list" id="record-list">
      ...
      {page.items.length ? (
        ...
        <div className="ledger-empty">
          ...
          <p>Record the first shared moment before adding an expense.</p>
        </div>
      )}
      <RecordPagination
        page={page.page}
        ...
        href={recordHref(path, query)}
      />
    </div>
  );
}

function OrganizationOutingCreatePanel({
  data,
  organizationId,
}: {
  data: OrganizationOutingsData;
  ...
  return (
    ...
    >
      <OutingForm
        action={createOutingAction.bind(null, organizationId, undefined)}
        initialOccurredAtUtc={new Date().toISOString()}
        trips={[
          { id: "", label: "No trip" },
          ...(selectedTrip ? [{ id: selectedTrip.id, label: selectedTrip.name }] : []),
        ]}
...
function OrganizationOutingsContent({
  data,
  organizationId,
  base,
  path,
    ...
    <section className="app-page outings-page" id="top">
      <div className="editorial-shell app-page__layout">
        <div className="app-page__header">
          <div>
            <p className="technical-label">Organization outings · shared events</p>
            <h1>Outings</h1>
            <p className="app-page__lede">
              Keep related expenses together under the event where they happened.
            </p>
          </div>
          {canManage ? (
            <Link
        ...
        <div className="records-workspace">
          <div className="records-workspace__toolbar">
            <OutingsTripsSwitch current="outings" basePath={base} />
            <LiveRecordFilters
              action={path}
              search={{
              ...
              selects={[
                {
                  ...
                  search: searchTripFilterOptions.bind(null, organizationId),
                },
              ]}
              ...
              resultStatus={
                page.totalItems + " outing" + (page.totalItems === 1 ? "" : "s") + " found."
              }
              preservedParams={query}
            />
          </div>
          <OrganizationOutingList
            data={data}
            base={base}
            path={path}
            query={query}
      ...
      <OrganizationOutingCreatePanel data={data} organizationId={organizationId} />
    </section>
  );
}

export default async function OrganizationOutingsPage({
  params,
