...
const mocks = vi.hoisted(() => ({ requireSession: vi.fn(), getDatabase: vi.fn(), getOrganizationForMember: vi.fn(), listOrganizationParticipants: vi.fn(), listPendingOrganizationInvitations: vi.fn() }));

vi.mock("server-only", () => ({}));
vi.mock("@/auth/require-session", () => ({ requireSession: mocks.requireSession }));
vi.mock("@/db/client", () => ({ getDatabase: mocks.getDatabase }));
vi.mock("@/server/organizations", () => ({ getOrganizationForMember: mocks.getOrganizationForMember }));
vi.mock("@/server/organization-invitations", () => ({ listPendingOrganizationInvitations: mocks.listPendingOrganizationInvitations }));
vi.mock("@/server/organization-participants", () => ({ listOrganizationParticipants: mocks.listOrganizationParticipants }));
...
describe("Organization People capability composition", () => {
  beforeEach(() => {
    ...
    mocks.listOrganizationParticipants.mockResolvedValue([]);
  ...
  it("does not expose expense contacts to members.view alone", async () => {
    mocks.getOrganizationForMember.mockResolvedValue({ id: "org-a", name: "Studio", description: null, canViewMembers: true, canManageMembers: false, canViewLedger: false, invitationRoles: [] });
