...
export default async function OrganizationExportsPage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  return <section className="app-page exports-page" id="top"><div className="editorial-shell app-page__layout"><div className="app-page__header"><div><p className="technical-label">Organization exports</p><h1>Exports</h1><p className="app-page__lede">Download this Organization’s ledger data.</p></div></div>{access.can("exports.create") ? <div className="exports-list">{exportsList.map(([title, description, kind]) => <div className="exports-row" key={kind}><div><h2>{title}</h2><p>{description}</p></div><Link className="text-link" href={`${base}/exports/${kind}`}>Download CSV <span aria-hidden="true">→</span></Link></div>)}</div> : <p className="app-page__lede">Exports are not enabled for your Organization access.</p>}</div></section>;
}
