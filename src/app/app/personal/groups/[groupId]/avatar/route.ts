...
import { deleteGroupAvatar, getGroupAvatar, GroupError, saveGroupAvatar } from "@/server/groups";
...
function contentLengthError(request: Request) {
  ...
  return length > MAX_REQUEST_BYTES ? "Group avatar upload requests must be 6 MiB or smaller." : null;
}
...
function accessStatus(error: unknown) { return error instanceof GroupError && error.code === "forbidden" ? 403 : 404; }

export async function GET(request: Request, { params }: { params: Promise<{ groupId: string }> }) {
  const current = await session();
  if (!current) return new Response("Unauthorized", { status: 401, headers: privateHeaders() });
  try {
    const avatar = await getGroupAvatar(getDatabase(), (await params).groupId, current.user.id);
...
export async function POST(request: Request, { params }: { params: Promise<{ groupId: string }> }) {
  ...
  if (lengthError) return json({ error: lengthError }, lengthError.startsWith("Group avatar") ? 413 : 400);
  ...
  try {
    ...
    const avatar = await saveGroupAvatar(getDatabase(), (await params).groupId, current.user.id, normalized);
    return json({ avatar });
  } catch (error) {
    if (error instanceof AvatarFileValidationError) return json({ field: "avatar", error: error.message }, 400);
    if (error instanceof GroupError) return new Response("Group unavailable.", { status: accessStatus(error), headers: privateHeaders() });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ groupId: string }> }) {
  ...
  if (!current) return new Response("Unauthorized", { status: 401, headers: privateHeaders() });
  if (!isSameOriginRequest(request)) return new Response(SAME_ORIGIN_ERROR, { status: 403 });
  try { await deleteGroupAvatar(getDatabase(), (await params).groupId, current.user.id); return new Response(null, { status: 204, headers: privateHeaders() }); }
  catch (error) { return new Response("Group unavailable.", { status: accessStatus(error), headers: privateHeaders() }); }
}
