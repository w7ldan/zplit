...
function GroupExpenseFilters({
  ...
  return (
    <form className="group-expense-filters" method="get" action={path}>
      <label htmlFor="group-expense-search">Search expenses</label>
      <input
        id="group-expense-search"
        name="q"
        type="search"
        defaultValue={search ?? ""}
        placeholder="Description"
      />
      <label htmlFor="group-expense-state">State</label>
      <select
        id="group-expense-state"
        name="state"
        defaultValue={state}
      >
...
function GroupExpenseList({
  ...
  page: RecordPage<GroupExpenseListRecord>;
  path: string;
  listPath: string;
  viewerUserId: string;
  filtered: boolean;
    ...
    <div className="ledger-list" id="record-list">
      ...
      {page.items.length ? (
        ...
        <div className="ledger-empty">
          ...
          <p>
            {filtered
              ? "Try a different description or state."
              : "Record the first shared expense inside this Group."}
          </p>
          {!filtered ? (
            <Link
              className="text-link"
              href={`${path}?create=1`}
              data-task-trigger="group-expense-create"
            >
...
function GroupExpenseCreatePanel({
  groupId,
  participants,
  defaultPayerId,
  query,
  ...
  return first(query.create) === "1" ? (
    ...
    >
      <GroupExpenseForm
        action={createGroupExpenseAction.bind(null, groupId)}
...
export default async function GroupExpensesPage({
  ...
  searchParams?: Promise<GroupExpenseSearchParams>;
}) {
  ...
  let participants;
  try {
    [page, participants] = await Promise.all([
      repository.listExpenses(session.user.id, first(query.page), {
        q: search,
        state: expenseState,
      }),
      repository.getParticipantEligibility(session.user.id),
    ]);
  } catch (error) {
    if (
      error instanceof GroupAccountingError &&
      (error.code === "not_member" || error.code === "invalid_id")
    ) notFound();
    throw error;
  }
      ...
      <div className="editorial-shell app-page__layout">
        <header className="app-page__header">
          <div>
            ...
            <h1>Expenses</h1>
            <p className="app-page__lede">Shared spending recorded inside this Group. Confirmed expenses create participant-to-participant obligations.</p>
          </div>
          <Link
