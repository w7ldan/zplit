...
const lifecycleLabels = {
  created: "Created by",
  payer_confirmed: "Confirmed by",
  payer_rejected: "Rejected by",
  voided: "Voided by",
} as const;
...
function lifecycleActor(expense: Pick<GroupExpenseDetail, "creator" | "payer" | "shares">, actorUserId: string) {
  return [
    expense.creator,
    expense.payer,
    ...expense.shares.map((share) => share.participant),
  ]
    .find((participant) => participant.userId === actorUserId)
...
function GroupExpenseStatus({
  ...
  return (
    <section
      className={`group-expense__status group-expense__status--${expense.state}`}
      id="group-expense-status"
      tabIndex={-1}
      aria-labelledby="group-expense-status-heading"
    >
      <div>
        <p className="technical-label">STATE</p>
        <h2 id="group-expense-status-heading">{stateLabels[expense.state]}</h2>
      </div>
      <p>{statusCopy(expense, activePayer)}</p>
      {expense.state === "pending" && activePayer ? (
        <GroupExpenseConfirmation
          confirmAction={confirmGroupExpenseAction.bind(
            null,
            groupId,
            expense.id,
          )}
          rejectAction={rejectGroupExpenseAction.bind(
            null,
            groupId,
            expense.id,
          )}
      ...
      {expense.state === "confirmed" && activePayer ? (
        <GroupExpenseVoid
          action={voidGroupExpenseAction.bind(null, groupId, expense.id)}
        />
      ) : null}
    </section>
  );
...
function GroupExpenseObligations({
  ...
  return (
    <section
      ...
      aria-labelledby="group-expense-obligations-heading"
    >
      <div className="group-section-heading">
        <div>
          <p className="technical-label">
            {historical ? "HISTORICAL OBLIGATIONS" : "OBLIGATIONS"}
          </p>
          <h2 id="group-expense-obligations-heading">
            {historical
              ? "Previous participant debt"
              : "Participant-to-participant debt"}
          </h2>
        </div>
      </div>
      {canShow && expense.obligations.length ? (
        <div
          className={`group-expense__obligation-list${
            historical ? " group-expense__obligation-list--historical" : ""
          }`}
        >
          {expense.obligations.map((obligation) => (
            <div
              className="group-expense__obligation-row"
              key={obligation.id}
            >
              <span>
                <GroupParticipantLabel
                  participant={obligation.debtor}
                /> owes <GroupParticipantLabel
                  participant={obligation.creditor}
                />
              </span>
              <span>
        ...
        <p className="group-expense__supporting-copy">{emptyCopy}</p>
      )}
    </section>
  );
}
...
function GroupExpenseHistory({ expense }: { expense: GroupExpenseDetail }) {
  ...
  return (
    <section
      className="group-expense__section group-expense__history"
      aria-labelledby="group-expense-history-heading"
    >
      <div className="group-section-heading">
        <div>
          <p className="technical-label">HISTORY</p>
          <h2 id="group-expense-history-heading">Lifecycle history</h2>
        </div>
      </div>
      {events.length ? (
        <ol className="group-expense__history-list">
          {events.map((event) => (
            <li key={event.id}>
              <span>
                {lifecycleLabels[event.eventType]} {lifecycleActor(expense, event.actorUserId)}
              </span>
        ...
        <p className="group-expense__supporting-copy">
          No lifecycle history is available for this record.
        </p>
      )}
    </section>
  );
  ...
  return (
    <aside className="group-expense-record__sidebar">
      <dl className="group-expense__meta">
        <div>
          <dt>State</dt>
          <dd>{stateLabels[expense.state]}</dd>
        </div>
        <div>
          <dt>Total</dt>
        ...
        <div>
          <dt>Created by</dt>
          <dd>
            <GroupParticipantLabel participant={expense.creator} />
          </dd>
        </div>
        <div>
          <dt>Paid by</dt>
          <dd>
            <GroupParticipantLabel participant={expense.payer} />
          </dd>
        </div>
        {expense.confirmedAt ? (
          <div>
            <dt>Confirmed</dt>
            <dd>
              <LocalDateTime iso={expense.confirmedAt.toISOString()} />
            </dd>
          </div>
        ) : null}
      </dl>
...
export default async function GroupExpenseDetailPage({
  ...
  let expense;
  try {
    expense = await createGroupAccountingRepository(
      getDatabase(),
      groupId,
    ).getExpense(expenseId, session.user.id);
  } catch (error) {
    if (
      error instanceof GroupAccountingError &&
      ["not_found", "not_member", "invalid_id"].includes(error.code)
    ) notFound();
    throw error;
  }
  ...
  return (
    <section className="app-page group-expense-record" id="top">
      <div className="editorial-shell app-page__layout">
        ...
        />
        <header className="group-expense-record__header">
          <div>
        ...
        <div className="group-expense-record__workspace">
          <main className="group-expense-record__main">
            ...
            <section
              className="group-expense__section"
              aria-labelledby="group-expense-shares-heading"
            >
              <div className="group-section-heading">
                <div>
                  <p className="technical-label">SPLIT</p>
                  <h2 id="group-expense-shares-heading">Shares</h2>
                </div>
                <strong>{formatRupiah(expense.totalAmount)}</strong>
              </div>
              <div className="group-expense__share-list">
                {expense.shares.map((share) => (
                  <div
                    className="group-expense__share-row"
                    key={share.id}
                  >
                    <span>
                      <GroupParticipantLabel participant={share.participant} />
                    </span>
                    <strong>{formatRupiah(share.amount)}</strong>
                  </div>
                ))}
              </div>
            </section>
            ...
            <ExpenseReceipts
              expenseId={expense.id}
              initialReceipts={expense.receipts.map((receipt) => ({
                ...receipt,
              ...
              readOnlyMessage={
                expense.state === "pending"
                  ? "Only the creator can change receipts while this expense is pending."
                  : "Receipts are read-only after an expense leaves pending state."
              }
            />
          </main>
