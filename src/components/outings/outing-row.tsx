    ...
    <article className={`outing-row${emphasized ? " outing-row--created" : ""}`} data-record-id={outing.id}>
      ...
      <div className="outing-row__meta">
        ...
        <span className="outing-row__actions">
          <Link className="outing-row__edit" href={`${basePath}/outings/${outing.id}`}>Edit <span aria-hidden="true">→</span></Link>
          <Link className="outing-row__add-expense" href={`${basePath}/expenses?create=1&outing=${outing.id}`}>Add expense</Link>
        </span>
      </div>
    </article>
  );
