...
function RepaymentAllocationSection({ draft, state, disclosureRef }: { draft: RepaymentAllocationDraft; state: RepaymentActionState; disclosureRef: React.RefObject<HTMLDetailsElement | null> }) {
  const {
    addAllocation,
  ...
  return <details ref={disclosureRef} open={allocationDisclosureOpen || undefined} className="repayment-form__disclosure">
    ...
    <section className="repayment-form__allocations" aria-labelledby="repayment-allocations-heading">
      ...
      {selectedAllocationRows.map((share) => (
        <div className="repayment-form__allocation" key={share.id}>
          ...
          <input id={"repayment-allocation-" + share.id} name="amountRupiah" type="text" inputMode="numeric" placeholder="Optional" value={allocationRows.find((allocation) => allocation.expenseShareId === share.id)?.amountRupiah ?? ""} onChange={(event) => updateAllocationAmount(share.id, event.target.value)} aria-invalid={Boolean(state.allocationFieldErrors?.[share.id])} />
          {state.allocationFieldErrors?.[share.id] ? <p className="repayment-form__field-error">{state.allocationFieldErrors[share.id]}</p> : null}
          <button className="action-link action-link--quiet repayment-form__allocation-remove" type="button" onClick={() => removeAllocation(share.id)}>Remove</button>
        </div>
      ))}
...
function RepaymentDetailsFields({ state, mode, detailsRef, detailsOpen, paymentMethodChoice, paymentMethodOther, recentPaymentMethods, onChoiceChange, onOtherChange }: {
  ...
  const fields = <>
    <div className="repayment-form__field">
      <PaymentMethodFields choice={paymentMethodChoice} other={paymentMethodOther} recentMethods={recentPaymentMethods} error={state.fieldErrors.paymentMethod} onChoiceChange={onChoiceChange} onOtherChange={onOtherChange} />
    </div>
    <div className="repayment-form__field">
  ...
  return mode === "create" ? <details ref={detailsRef} open={detailsOpen || undefined} className="repayment-form__disclosure"><summary>Optional details</summary>{fields}</details> : fields;
}

function RepaymentBasicFields({ state, searchFriends, friendLocked, selectedFriendId, friendOptionsWithSelection, handleFriendChange, outstandingAmount, tripContext, tripContextId, amountRef, handleAmountChange, loadingFriendContext, selectedContext, timezoneOffsetRef, mode }: {
  state: RepaymentActionState;
  searchFriends: SearchableOptionAction;
  friendLocked: boolean;
  ...
  return <>
    <div className="repayment-form__field">
      ...
      <SearchableCombobox id="repayment-friend" name="friendId" value={selectedFriendId} options={friendOptionsWithSelection} search={searchFriends} required={!friendLocked} disabled={friendLocked} placeholder="Choose friend" searchLabel="Search friends" ariaInvalid={Boolean(state.fieldErrors.friendId)} ariaDescribedBy="repayment-friend-error" labelId="repayment-friend-label" onValueChange={handleFriendChange} />
      <p className="repayment-form__outstanding" aria-live="polite">Outstanding for {friendOptionsWithSelection.find((friend) => friend.id === selectedFriendId)?.label ?? "this friend"}: {formatRupiah(outstandingAmount)}</p>
  ...
  return (
    ...
    >
      <RepaymentBasicFields
        ...
        mode={mode}
      />
      {mode === "create" ? <RepaymentAllocationSection draft={allocation} state={state} disclosureRef={allocationDisclosureRef} /> : null}
      <RepaymentDetailsFields state={state} mode={mode} detailsRef={detailsDisclosureRef} detailsOpen={detailsDisclosureOpen} paymentMethodChoice={paymentMethodChoice} paymentMethodOther={paymentMethodOther} recentPaymentMethods={recentPaymentMethods} onChoiceChange={setPaymentMethodChoice} onOtherChange={setPaymentMethodOther} />
