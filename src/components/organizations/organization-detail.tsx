...
export function OrganizationProfile({ organization, action }: { organization: { id: string; name: string; description: string | null; avatar: { sha256: string } | null }; action: ProfileAction }) {
  return <div className="organization-detail__owner-tools"><h2>Organization profile</h2><OrganizationForm action={action} edit initialValues={{ name: organization.name, description: organization.description ?? "" }} /><AvatarSettings userId={organization.id} avatar={organization.avatar} endpoint={`/app/organizations/${organization.id}/avatar`} avatarUrl={(avatar) => `/app/organizations/${organization.id}/avatar?v=${encodeURIComponent(avatar.sha256)}`}><p className="technical-label">PROFILE IMAGE</p></AvatarSettings></div>;
}
