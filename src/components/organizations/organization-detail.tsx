...
export function OrganizationProfile({ organization, action }: { organization: { id: string; name: string; description: string | null; avatar: { sha256: string } | null }; action: ProfileAction }) {
  return (
    <div className="organization-detail__owner-tools">
      ...
      <AvatarSettings
        ...
        defaultAvatarSeed={organizationAvatarSeed(organization.id)}
        avatar={organization.avatar}
        endpoint={`/app/organizations/${organization.id}/avatar`}
        avatarUrl={(avatar) =>
          `/app/organizations/${organization.id}/avatar?v=${encodeURIComponent(avatar.sha256)}`
        }
      >
        <p className="technical-label">PROFILE IMAGE</p>
      </AvatarSettings>
    </div>
  );
}
...
export function OrganizationIdentity({ organization }: { organization: { id: string; name: string; avatar: { sha256: string } | null } }) {
  return (
    <div className="organization-detail__identity">
      ...
      />
      <div>
        <p className="technical-label">ORGANIZATION</p>
...
export function OrganizationNavigation({ organizationId, canViewLedger, canViewChat = false, chatUnreadCount = 0, canViewPeople, canViewSettings }: { organizationId: string; canViewLedger: boolean; canViewChat?: boolean; chatUnreadCount?: number; canViewPeople: boolean; canViewSettings: boolean }) {
    ...
    ...(canViewLedger
      ? [
          [
            "Activity",
            `${base}/trips`,
            pathname.startsWith(`${base}/trips`) ||
              pathname.startsWith(`${base}/outings`),
          ],
          [
            "Expenses",
            `${base}/expenses`,
          ...
          [
            "Repayments",
            `${base}/repayments`,
            pathname.startsWith(`${base}/repayments`),
          ],
  ...
  return <>
    ...
    {activity ? (
      <nav
        className="organization-context__subnav"
        aria-label="Organization activity navigation"
      >
        <Link
          href={`${base}/trips`}
          aria-current={
            pathname.startsWith(`${base}/trips`) ? "page" : undefined
          }
        >
          Trips
        </Link>
        <Link
          href={`${base}/outings`}
          aria-current={
            pathname.startsWith(`${base}/outings`) ? "page" : undefined
          }
        >
          Outings
        </Link>
