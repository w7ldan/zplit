...
type FriendLinkAction = (
  previousState: FriendLinkActionState,
  formData: FormData,
...
function identityParts(label: string) {
  ...
  return separator === -1
    ...
    : {
        displayName: label.slice(0, separator),
        username: label.slice(separator + 3),
      };
}
...
export function FriendLinkSection({
  status,
  search,
  action,
  cancelAction,
        ...
        {unlinkAction ? (
          <details className="friend-link__unlink">
            ...
            <div>
              ...
              <p>
                This removes the Zplit account connection. Existing Friend balances and history remain unchanged.
              </p>
      ...
      {!open ? (
        ...
        <form className="friend-link__disclosure" action={formAction}>
          ...
          <SearchableCombobox
            ...
            labelId="friend-link-target-label"
            onValueChange={setSelected}
          />
          {selectedIdentity ? (
            <div className="friend-link__confirm">
              <p className="technical-label">Link this Friend to</p>
              <strong>{selectedIdentity.displayName}</strong>
              <span>{selectedIdentity.username}</span>
            </div>
          ) : null}
          <p
            className="friend-link__message"
