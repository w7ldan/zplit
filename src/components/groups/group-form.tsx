...
function SubmitButton({ edit }: { edit: boolean }) {
  ...
  return (
    <button
      ...
      aria-busy={pending}
    >
      {pending ? "Saving…" : edit ? "Save changes" : "Create group"}
    </button>
  );
}
...
export function GroupForm({
  ...
  const [state, formAction] = useActionState(action, {
    fieldErrors: {},
    formError: "",
    values: initialValues ?? { name: "", description: "" },
  });
  ...
  return (
    <form
      ...
      action={formAction}
      key={`${state.values.name}\u0000${state.values.description}`}
      noValidate
    >
      <div className="group-form__field">
        ...
        />
        <p className="group-form__field-error" id="group-name-error">
      ...
      <div className="group-form__field">
        ...
        />
        <p className="group-form__field-error" id="group-description-error">
      ...
      {!edit ? (
        <div className="group-form__field">
          ...
          <input
            ...
            aria-describedby="group-avatar-help group-avatar-error"
          />
          <p className="group-form__help" id="group-avatar-help">
            JPEG, PNG, or WebP. Zplit will normalize it safely.
          </p>
          <p className="group-form__field-error" id="group-avatar-error">
            {state.fieldErrors.avatar || "\u00a0"}
          </p>
        </div>
      ) : null}
      <p
        className="group-form__message"
        role={state.formError && state.formError !== "Profile saved." ? "alert" : "status"}
        aria-live="polite"
      >
        {state.formError || "\u00a0"}
      </p>
