...
type GroupExpenseAction = (
  ...
  formData: FormData,
) => Promise<GroupExpenseActionState>;
...
function FieldError({ id, message }: { id: string; message?: string }) {
  return (
    <p className="group-expense-form__field-error" id={id}>
      {message || "\u00a0"}
    </p>
  );
}
...
export function GroupExpenseForm({
  ...
  const initialValues: GroupExpenseFormValues = {
    description: "",
    totalAmount: "",
    occurredAtLocal: "",
  ...
  useEffect(() => {
    ...
    if (
      occurredInput instanceof HTMLInputElement &&
      !occurredInput.value &&
      initialOccurredAtUtc
    ) occurredInput.value = localValueFromUtc(initialOccurredAtUtc);
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!total || assigned !== total || !shares.length) {
      ...
      setLocalError(
        !shares.length
          ? "Add at least one participant share."
          : assigned > total
            ? `Over-allocated by ${formatRupiah(assigned - total)}.`
            : `Allocate ${formatRupiah(total - assigned)} more.`,
      );
    }
  }
    ...
    >
      <div className="group-expense-form__field">
        ...
        <input
          ...
          aria-describedby="group-expense-description-error"
        />
        <FieldError
          id="group-expense-description-error"
      ...
      <div className="group-expense-form__field">
        ...
        <input
          ...
          aria-invalid={Boolean(state.fieldErrors.totalAmount)}
          aria-describedby="group-expense-total-help group-expense-total-error"
          autoComplete="off"
        />
        <p className="group-expense-form__help" id="group-expense-total-help">
          Whole rupiah only. Examples: 100000 or 100.000.
        </p>
        <FieldError
          id="group-expense-total-error"
          message={state.fieldErrors.totalAmount}
        />
      </div>
      <div className="group-expense-form__field">
        <label htmlFor="group-expense-occurred-at">Occurred date and time</label>
        <input
          id="group-expense-occurred-at"
          name="occurredAtLocal"
          type="datetime-local"
          defaultValue={state.values.occurredAtLocal}
          aria-invalid={Boolean(state.fieldErrors.occurredAtLocal)}
          aria-describedby="group-expense-occurred-at-error"
        />
        <input
          type="hidden"
        ...
        />
        <FieldError
          id="group-expense-occurred-at-error"
          message={state.fieldErrors.occurredAtLocal}
        />
      </div>
      <div className="group-expense-form__field">
        ...
        <select
          ...
          name="payerParticipantId"
          value={payerParticipantId}
          onChange={(event) => setPayerParticipantId(event.target.value)}
          aria-invalid={Boolean(state.fieldErrors.payerParticipantId)}
          aria-describedby="group-expense-payer-help group-expense-payer-error"
        >
        ...
        <p className="group-expense-form__help" id="group-expense-payer-help">
          Choosing another member records their payment claim. They must confirm before debt is created.
        </p>
        <FieldError
          id="group-expense-payer-error"
      ...
      >
        ...
        <p className="group-expense-form__help" id="group-expense-shares-help">
          Active members and external participants may receive a whole-rupiah share. The payer may also have a share.
        </p>
        {shares.length ? (
          <div className="group-expense-form__share-list">
            {shares.map((share) => {
              const participant = shareOptions.find(
                (option) => option.id === share.participantId,
              );
              return (
                <div
                  className="group-expense-form__share-row"
                  key={share.participantId}
                >
                  <select
                    name="participantId"
                    value={share.participantId}
                    onChange={(event) =>
                      setShares((current) =>
                        current.map((item) =>
                          item.participantId === share.participantId
                            ? { ...item, participantId: event.target.value }
                            : item,
                        ),
                      )
                    }
                    aria-label={`Share participant ${participant?.displayName ?? ""}`}
                  >
                    {shareOptions
                      .filter(
                        (option) =>
                          option.id === share.participantId ||
                          !shares.some((item) => item.participantId === option.id),
                      )
                      .map((option) => (
                  ...
                  <input
                    ...
                    aria-label={`Share amount for ${participant?.displayName ?? "participant"}`}
                    autoComplete="off"
                  />
                  <button
                    className="text-link"
          ...
          <p className="group-expense-form__empty">No participants added yet.</p>
        )}
        <div className="group-expense-form__add-share">
          <select
            value={addParticipantId}
            onChange={(event) => setAddParticipantId(event.target.value)}
            aria-label="Participant to add"
          >
        ...
        <div className="group-expense-form__totals" aria-live="polite">
          <span>
            <span className="technical-label">Assigned</span>
            <strong>{formatRupiah(assigned)}</strong>
          </span>
          <span>
        ...
        <FieldError
          id="group-expense-shares-error"
          message={state.fieldErrors.shares || localError}
        />
      </fieldset>
      <p
        className="group-expense-form__message"
