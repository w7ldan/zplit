...
export function GroupProfile({ group, action }: { group: { id: string; name: string; description: string | null; avatar: { sha256: string } | null }; action: ProfileAction }) {
  return (
    <div className="group-detail__profile">
      <h2>Group profile</h2>
      <GroupForm
        action={action}
        edit
      ...
      <AvatarSettings
        userId={group.id}
        defaultAvatarSeed={groupAvatarSeed(group.id)}
        avatar={group.avatar}
        endpoint={`/app/personal/groups/${group.id}/avatar`}
        avatarUrl={(avatar) =>
          `/app/personal/groups/${group.id}/avatar?v=${encodeURIComponent(avatar.sha256)}`
        }
      >
  ...
  return (
    <nav className="group-context__nav" aria-label="Group navigation">
      ...
      <Link
        href={`${base}/expenses`}
        aria-current={
          pathname.startsWith(`${base}/expenses`) ? "page" : undefined
        }
      >
        Expenses
      </Link>
