...
function pageHref(href: string, page: number, anchor: string, pageParam: string) {
...
export function RecordPagination({ page, pageSize, totalItems, totalPages, href, anchor = "record-list", pageParam = "page" }: RecordPaginationProps) {
  ...
  return (
    <nav className="record-pagination" aria-label="Record pages">
      <span className={page <= 1 ? "record-pagination__disabled" : undefined} aria-disabled={page <= 1 ? "true" : undefined}>
        {page <= 1 ? "Previous" : <Link href={pageHref(href, page - 1, anchor, pageParam)}>Previous</Link>}
      </span>
      ...
      <span className={page >= totalPages ? "record-pagination__disabled" : undefined} aria-disabled={page >= totalPages ? "true" : undefined}>
        {page >= totalPages ? "Next" : <Link href={pageHref(href, page + 1, anchor, pageParam)}>Next</Link>}
      </span>
    </nav>
  );
}
