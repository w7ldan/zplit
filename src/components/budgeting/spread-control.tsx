...
export function SpreadControl({ transaction }: { transaction: BudgetTransactionView }) {
  ...
  return <details className="budget-category-change budget-spread-control">
    <summary className="action-link action-link--quiet">{transaction.spreadCount ? `Spread across ${transaction.spreadCount} periods` : "Spread across periods"}</summary>
    <BudgetSpreadForm action={spreadBudgetTransactionAction} transactionId={transaction.id} amount={transaction.amount} initialCount={String(transaction.spreadCount ?? 1)} />
  </details>;
}
