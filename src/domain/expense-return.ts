...
export function validateExpenseReturnTarget(value: unknown) {
  ...
  if (
    ...
    url.pathname !== "/app/expenses"
  ) return undefined;

  return value;
}

export function addOutingToExpenseReturnTarget(target: string, outingId: string) {
  if (!validateExpenseReturnTarget(target) || typeof outingId !== "string" || !outingId) return undefined;
  const url = new URL(target, PLACEHOLDER_ORIGIN);
  url.searchParams.set("create", "1");
  url.searchParams.set("outing", outingId);
