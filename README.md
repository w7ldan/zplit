...
Authentication supports multiple users structurally, every domain record is scoped to its owner, and the protected application now provides owner-scoped Friends and Outings management. Public registration remains disabled; production currently has one provisioned account. Expenses, repayments, allocation workflows, account provisioning, and temporary debtor links are later stages. The design contract is in `docs/design-system.md`.
...
The initial schema and authenticated Friends and Outings workflows are implemented. Expenses, repayments, and allocation workflows remain deferred to later checkpoints.
