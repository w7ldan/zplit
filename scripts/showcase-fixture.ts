...
async function insertShowcaseState(client: PoolClient, fixture: ShowcaseFixtureData, generateToken: typeof generateDebtorShareToken) {
  ...
  await insertRows(client, "expense_shares", ["id", "owner_user_id", "expense_id", "friend_id", "amount_owed", "created_at"], fixture.expenseShares.map((row) => [row.id, row.ownerUserId, row.expenseId, row.friendId, row.amountOwed, row.createdAt]));
