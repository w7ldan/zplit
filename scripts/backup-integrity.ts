...
export const INTEGRITY_CHECKS: readonly IntegrityCheck[] = [
  ...
  },
  {
    ...
    sql: `SELECT (
      ...
      (SELECT count(*) FROM group_offset_settlements s WHERE NOT EXISTS (SELECT 1 FROM groups g WHERE g.id = s.group_id) OR NOT EXISTS (SELECT 1 FROM group_participants p WHERE p.group_id = s.group_id AND p.id = s.initiator_participant_id AND p.user_id IS NOT NULL) OR NOT EXISTS (SELECT 1 FROM group_participants p WHERE p.group_id = s.group_id AND p.id = s.counterparty_participant_id AND p.user_id IS NOT NULL) OR (s.state = 'pending' AND s.confirmed_at IS NOT NULL) OR (s.state = 'confirmed' AND s.confirmed_at IS NULL)) +
      (SELECT count(*) FROM group_offset_applications a WHERE NOT EXISTS (SELECT 1 FROM group_offset_settlements s WHERE s.group_id = a.group_id AND s.id = a.offset_settlement_id AND s.state = 'confirmed') OR NOT EXISTS (SELECT 1 FROM group_obligations o WHERE o.group_id = a.group_id AND o.id = a.obligation_id) OR EXISTS (SELECT 1 FROM group_offset_settlements s JOIN group_obligations o ON o.group_id = a.group_id AND o.id = a.obligation_id WHERE s.group_id = a.group_id AND s.id = a.offset_settlement_id AND NOT (((o.debtor_participant_id = s.initiator_participant_id AND o.creditor_participant_id = s.counterparty_participant_id) OR (o.debtor_participant_id = s.counterparty_participant_id AND o.creditor_participant_id = s.initiator_participant_id)) AND o.created_at <= s.confirmed_at AND (o.voided_at IS NULL OR o.voided_at > s.confirmed_at)))) +
      (SELECT count(*) FROM (SELECT s.group_id, s.id, s.amount FROM group_offset_settlements s LEFT JOIN group_offset_applications a ON a.group_id = s.group_id AND a.offset_settlement_id = s.id LEFT JOIN group_obligations o ON o.group_id = a.group_id AND o.id = a.obligation_id WHERE s.state = 'confirmed' GROUP BY s.group_id, s.id, s.amount HAVING COALESCE(sum(a.applied_amount) FILTER (WHERE o.debtor_participant_id = s.initiator_participant_id AND o.creditor_participant_id = s.counterparty_participant_id), 0) <> s.amount OR COALESCE(sum(a.applied_amount) FILTER (WHERE o.debtor_participant_id = s.counterparty_participant_id AND o.creditor_participant_id = s.initiator_participant_id), 0) <> s.amount) invalid_offsets) +
      (SELECT count(*) FROM (SELECT s.group_id, s.id AS settlement_id, s.amount FROM group_settlements s LEFT JOIN group_settlement_applications a ON a.group_id = s.group_id AND a.settlement_id = s.id WHERE s.state = 'confirmed' GROUP BY s.group_id, s.id, s.amount HAVING COALESCE(sum(a.applied_amount), 0) <> s.amount) invalid_settlements) +
      (SELECT count(*) FROM group_obligations o WHERE (SELECT COALESCE(sum(s.applied_amount), 0) FROM group_settlement_applications s WHERE s.group_id = o.group_id AND s.obligation_id = o.id) + (SELECT COALESCE(sum(f.applied_amount), 0) FROM group_offset_applications f WHERE f.group_id = o.group_id AND f.obligation_id = o.id) > o.original_amount) +
