...
describe("backup integrity", () => {
  ...
  it("keeps all expected tables and integrity SQL checks explicit", () => {
    expect(EXPECTED_TABLES).toEqual(expect.arrayContaining([
      "users", "account_invitations", "ledger_scopes", "debtor_share_links", "friends", "expenses", "expense_receipts", "repayments", "repayment_destinations", "repayment_proofs", "repayment_allocations",
    ]));
    for (const name of ["scope-aware foreign keys", "shares within expenses", "allocations within repayments", "allocations within shares", "cross-friend allocations", "accepted and revoked invitations", "duplicate active debtor links", "whole-rupiah values", "receipt byte lengths"]) {
