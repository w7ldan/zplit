...
describe("Motion and feedback contract", () => {
  ...
  it("keeps interaction motion pointer-safe and tokenized", () => {
    ...
    expect(motionSource).toContain("transition: background-color var(--motion-state) var(--ease-product);");
    expect(motionSource).toContain(".receipt-preview--entering .receipt-preview__surface");
    ...
    expect(recordsAndFormsSource).toContain("transform-origin: bottom center;");
    expect(recordsAndFormsSource).toContain("scale(0.99)");
    expect(recordsAndFormsSource).toMatch(/@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.searchable-combobox__panel,[\s\S]*?\.searchable-combobox__panel--closing\s*\{[\s\S]*?opacity: 1;[\s\S]*?transform: none;[\s\S]*?transition: none !important;/);
