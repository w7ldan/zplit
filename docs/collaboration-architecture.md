...
The server publishes wake-ups through PostgreSQL `LISTEN/NOTIFY`, with one
shared listener per effective web runtime fanning out to user-scoped SSE
subscribers. PostgreSQL remains canonical state; `LISTEN/NOTIFY` is only a
freshness adapter and SSE remains browser delivery. Consumers must refetch
canonical state after a reconnect or relevant invalidation; the transport does
