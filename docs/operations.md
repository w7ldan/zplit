...
The legacy capture fixture is disposable and uses `zplit_showcase`, a dedicated owner, and `ZPLIT_SHOWCASE_CONFIRM=showcase-only`. The repository-wide capture profile uses a separately named disposable database, `zplit_repository_showcase`, with the same confirmation guard and secret-file model. Configure credentials through regular secret files, run the app on its separate local port, and use only the `showcase:setup`, `showcase:state`, `showcase:verify`, `showcase:clear`, and `showcase:repository` commands for the fixture workflows. Read-only verification does not require mutation confirmation; setup, state changes, and clearing do. The repository profile and its exact capture routes are documented in [Showcase](showcase.md).

The legacy state commands replace only the showcase owner’s ledger, receipts, and share-link records in a guarded transaction and retain the dummy login. The repository profile validates its exact four fixture-owned identities, resets only `zplit_repository_showcase`, applies migrations, and recreates its synthetic account/data universe; this separate database is required to respect immutable confirmed Group financial history without bypassing constraints. After capture, clear the selected profile and drop the isolated database separately if no longer needed. If the database name or confirmation value is wrong, stop: do not remove guards or substitute another database.
