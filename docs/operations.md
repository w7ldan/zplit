...
The legacy state commands replace only the showcase owner’s ledger, receipts, and share-link records in a guarded transaction and retain the dummy login. The repository profile validates its known fixture-owned identities, resets only `zplit_repository_showcase`, applies migrations, and recreates its synthetic account/data universe; this separate database is required to respect immutable confirmed Group financial history without bypassing constraints. After capture, clear the selected profile and drop the isolated database separately if no longer needed. If the database name or confirmation value is wrong, stop: do not remove guards or substitute another database.
